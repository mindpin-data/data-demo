...
(function() {
  ...
  LineChartTitle = (function(superClass) {
    ...
    LineChartTitle.prototype.draw_texts = function() {
      ...
      texts.append('text').attr('x', -1050 + 270).attr('y', this.height / 2).attr('dy', '.33em').text(889718890).style('font-size', size * 1.5).style('fill', this.number_color);
      texts.append('text').attr('x', -1050 + 600).attr('y', this.height / 2).attr('dy', '.33em').text('产品出口销量').style('font-size', size).style('fill', '#ffffff');
      texts.append('text').attr('x', -1050 + 600 + 270).attr('y', this.height / 2).attr('dy', '.33em').text(142210067).style('font-size', size * 1.5).style('fill', this.number_color);
      size = 20;
...
(function() {
  var PathMap, max_number, people_data,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  people_data.forEach(function(x) {
  ...
  PathMap = (function(superClass) {
    ...
    PathMap.prototype.draw_map = function() {
      ...
      this.svg.style('z-index', '100').style('position', 'relative');
    ...
    PathMap.prototype._draw_map = function() {
      ...
      })(this)).style('stroke-width', 1).style('fill', (function(_this) {
        return function(d) {
          return 'transparent';
        };
      })(this));
    ...
    PathMap.prototype.draw_heatmap = function() {
      var data, heatmapInstance, points;
      heatmapInstance = h337.create({
        container: jQuery('#heatmap')[0],
        radius: 150 * this.map_scale,
        gradient: {
          '0.1': '#28669b',
          '0.9': '#34cee9',
          '1.0': 'white'
      ...
      points = [];
      people_data.forEach((function(_this) {
        return function(p) {
          var point, ref, x, y;
    ...
    PathMap.prototype.draw_circles = function() {
      return people_data.forEach((function(_this) {
        return function(p) {
    ...
    PathMap.prototype.random_country = function() {
      return setInterval((function(_this) {
        return function() {
          var feature, idx, ref, x, y;
          idx = Math.floor(_this.features.length * Math.random());
          feature = _this.features[idx];
    ...
    PathMap.prototype.show_city_animate = function(x, y, color, width) {
      var circle0, circle1, circle2, gyx, gyy, line, ref;
      ref = this.projection([106.4, 26.3]), gyx = ref[0], gyy = ref[1];
      line = this.g_map.append('line').attr('class', 'running').attr('x1', gyx).attr('y1', gyy).attr('x2', x).attr('y2', y).style('stroke', color).style('stroke-width', 4).style('stroke-dasharray', '200 200').style('stroke-linecap', 'round');
      circle0 = this.g_map.append('circle').attr('class', 'warning').attr('cx', x).attr('cy', y).attr('stroke', color).attr('stroke-width', width).attr('fill', 'transparent');
      circle1 = this.g_map.append('circle').attr('class', 'warning a').attr('cx', x).attr('cy', y).attr('stroke', color).attr('stroke-width', width).attr('fill', 'transparent');
      circle2 = this.g_map.append('circle').attr('class', 'warning b').attr('cx', x).attr('cy', y).attr('stroke', color).attr('stroke-width', width).attr('fill', 'transparent');
